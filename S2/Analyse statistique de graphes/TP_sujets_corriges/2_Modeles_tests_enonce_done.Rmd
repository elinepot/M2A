...
Dans ce TP nous allons utiliser des modèles de graphes aléatoires très simples pour tester des hypothèses sur les graphes. 
...
print("mean degree")
mean(degree.distribution(G))



degree.df <- data.frame(deg=0:max(degree(G,mode="in")), freq=degree_distribution(G,mode="in"))
p <- ggplot(degree.df, aes(x=deg, y=freq)) + geom_bar(stat="identity",color="blue")
#p + geom_hline(yintercept=n*p, 'dashed', color = 'purple')
p + ggtitle("Alldegree Distribution")
```
...
### Exercice 6 
Le but de cet exercice est d'analyser l'impact des paramètres sur les graphes obtenus avec un modèle par attachement préférentiel.
\begin{enumerate}
1. Écrire une fonction pour générer un graphe  selon le modèle d'attachement  préférentiel. Vous pourrez utiliser comme graphe
  initial un graphe $G(n,p)$.
2. Faites varier les paramètres (nombre d'itérations ; ordre du graphe  initial ; probabitité de connection du graphe initial ;  nombre d'arêtes à ajouter à chaque itération) et observez la suite des     degrés des n\oe uds du graphe. Qu'en pensez-vous ? 
