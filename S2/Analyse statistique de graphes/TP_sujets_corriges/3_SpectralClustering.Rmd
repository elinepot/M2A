...
Le but de cette partie  est d'implémenter  différents algorithmes de spectral clustering et d'analyser leur comportement sur quelques graphes simples. 
...
2. Simuler un petit graphe dont vous connaissez les communautés et tester votre fonction. On pourra prendre par exemple un graphe avec plusieurs composantes connexes, chacune étant de loi $G(n,p)$ avec $p$ assez grand. Puis ajouter des arêtes pour rendre ce graphe connexe.  
3. Faites de même pour l'**absolute spectral clustering** basé sur $L_\text{abs}$.

### Exercice 2
Simuler différents  graphes et analyser le spectre des laplaciens $L_N$ et $L_{\text{abs}}$ associés. Est-ce que la méthode du trou spectral (eigengap) permet de choisir un nombre de clusters raisonnable ?
Vérifier quelle méthode retrouve le bon clustering. D'ailleurs, comment compare-t-on les clusters obtenus par deux  algorithmes de  classification différents  ? (Il  faut faire attention au problème du ``label switching'').
