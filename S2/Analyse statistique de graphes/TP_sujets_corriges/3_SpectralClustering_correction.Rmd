...
Dans `igraph`, il semble qu'il existe une fonction permettant de faire le plongement (embedding) du graphe dans l'espace des premiers vecteurs propres : `embed_laplacian_matrix()`. Mais vous verrez ci-dessous que ne n'est pas très clair de savoir ce qu'elle fait exactement. 
...
Pour ce faire, au lieu d'écrire directement une fonction complexe, considérons d'abord un exemple simple pour lequel nous effectuons les différentes étapes une à une.
...
Nota Bene: ça ne donne pas exactement la même chose que 
```{r}
U <- embed_laplacian_matrix(G,no=3,which="sa",type="I-DAD",scaled=FALSE)$X
round(U,digits=3)
```
Je ne sais pas bien ce que fait cette fonction `embed_laplacian_matrix` (les options sont peu claires).
...
## Application à un graphe bipartie 
...
Ce clustering normalisé ne voit pas du tout les 2 parties. 
...
On constate que l'absolute spectral clustering est très sensible aux morceaux bi-parties du graphe. 
