...
author: "Catherine Matias and Tabea Rebafka"
date: "Février 2024"
output:
  html_document:
    df_print: paged
...
Les courbes théoriques se superposent bien aux barplots empiriques (bon sur la loi Poisson, le fait de tronquer l'axe des abscisses à droite masque une partie du non-ajustement). On n'observe pas de degrés très élevé. 
...
Les fonctions qu'on a écrites ne sont pas optimisées, il est préférable d'utiliser la fonction 'sample_degseq' de igraph. Attention aux options de cette fonction, par défaut, elle crée des graphes avec potentiellement des boucles et des arêtes multiples. La méthode la plus efficace est 'vl' (algorithme de Viger et Latapy).
```{r}
...
## 4. Attachement préférentiel 

### Exercice 6 

Voici le code pour générer la matrice d'adjacence par attachement préférentiel.

```{r}
sample_attach_pref <- function(n0, p0, m, nb.it){
  # ARGUMENTS : 
  # n0 - nb de noeuds du graphe initial
  # p0 - probabilité du G(n0,p0) initial
  # m - nb d'arêtes à ajouter à chaque itérations
  # nb.it - nb d'itérations de l'algo
  
  # Générer un graphe initial G(n0,p0) 
  G <- sample_gnp(n0, p0)

  for (t in 1:nb.it){
   # tirer m noeuds avec des proba proportionnelles aux degrés des noeuds
   ind <- sample(1:vcount(G), m, replace = FALSE, degree(G))   
   # ajouter le nouveau noeud et les nouvelles arêtes au graphe
   G <- add_vertices(G, 1)
   G <- add.edges(G, matrix(c(rep(vcount(G), m), ind), nrow = 2, byrow = TRUE))
  }
  return(G)   
}
```

Voyons ce que ça donne sur un petit exemple
```{r}
set.seed(2)
G <- sample_attach_pref(5, .5, 3, 10)
plot(G)
degres <- degree.distribution(G)
barplot(degres, names.arg = as.character(0:max(degree(G))), 
        main = 'Distribution des degrés')
```

Le graphe initial est relativement dense (car $p=0.5$) et on ajoute toujours des noeuds de degrés $m=3$. Par conséquent, le graphe final contient beaucoup de noeuds de degrés au moins 3.

Si on fait plus d'itérations :
```{r}
G <- sample_attach_pref(5, .5, 3, 100)
degres <- degree.distribution(G)
barplot(degres, names.arg = as.character(0:max(degree(G))), 
        main = 'Distribution des degrés')
```

Quelques noeuds se détachent des autres et deviennent des noeuds fortement connectés.


Si on fait encore plus d'itérations :
```{r}
G <- sample_attach_pref(5, .5, 3, 1000)
degres <- degree.distribution(G)
barplot(degres, names.arg = as.character(0:max(degree(G))), 
        main = 'Distribution des degrés')
```

La distribution de degrés des noeuds ressemble à une loi de puissance : la plupart de noeuds sont faiblement connectés, et il y a quelques hubs dans le graphe.

Jouons avec les paramètres du graphe initial. Augmentons d'abord l'ordre du graphe initial :

```{r}
G <- sample_attach_pref(100, .5, 3, 100)
degres <- degree.distribution(G)
barplot(degres, names.arg = as.character(0:max(degree(G))), 
        main = 'Distribution des degrés')
```

Le degré moyen du graphe initial étant relativement élevé (environ $n_0p_0=100*0.5=50$) par rapport au degré $m$ des nouveaux noeuds ajoutés au fil de l'algorithme,  on distingue facilement deux groupes de noeuds : ceux qu'on vient d'ajouter dans les itérations sont à degré très faibles, les autres sont les noeuds du graphe initial.

Que se passe-t-il quand on fait plus d'itérations :

```{r}
G <- sample_attach_pref(100, .5, 3, 1000)
degres <- degree.distribution(G)
barplot(degres, names.arg = as.character(0:max(degree(G))), 
        main = 'Distribution des degrés')
```

On distingue toujours très clairement les deux groupes de noeuds (initiaux et ceux des itérations). Les noeuds ajoutés au fil du temps n'arrivent pas à "rattraper" les noeuds intiaux en termes de degrés.

Diminuons la probabilité $p$ du graphe $G(n,p)$ initial.


```{r}
G <- sample_attach_pref(100, .1, 3, 1000)
degres <- degree.distribution(G)
barplot(degres, names.arg = as.character(0:max(degree(G))), 
        main = 'Distribution des degrés')
```

Ici on ne peut pas distinguer les deux groupes de noeuds, et la distribution des degrés de noeuds ressemble  à une loi de puissance.

Voyons l'effet du choix du nombre $m$ d'arêtes à ajouter au graphe à chaque itération :

```{r}
G <- sample_attach_pref(100, .5, 30, 1000)
degres <- degree.distribution(G)
barplot(degres, names.arg = as.character(0:max(degree(G))), 
        main = 'Distribution des degrés')
```
...
Bien qu'on soit parti d'un graphe initial relativement dense ($p=0.5$), on ne peut pas distinguer deux groupes de noeuds et la distribution des degrés des noeuds ressemble à une loi de puissance. 

On a l'impression qu'il faut que $m$  soit de l'ordre du degré moyen du graphe initial, pour que les noeuds ajoutés "se mélangent" avec les noeuds initiaux en termes de degrés et la distribution des degrés de noeuds ressemble à une loi de puissance. Autrement, on pourra toujours distinguer deux groupes: les noeuds du graphe initial et les noeuds ajoutés dans les itérations. 
